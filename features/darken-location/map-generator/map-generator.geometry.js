...
export function buildBoundarySegmentPath(segments) {
  ...
  return segments
    ...
    .map(
      (segment) =>
        `M ${roundTo(segment.x1, 2)} ${roundTo(segment.y1, 2)} L ${roundTo(segment.x2, 2)} ${roundTo(segment.y2, 2)}`,
    )
    .join(" ");
}
...
export function isPureCaveMap(generatedMap) {
  ...
  return regions.every(
    (region) => getRegionSurfaceKind(region, generatedMap) === "cave",
  );
}
...
export function getCaveMapAccessesForGeometry(generatedMap) {
  ...
  const sources = [
    ...(Array.isArray(generatedMap?.dungeonMask?.mapAccesses)
      ? generatedMap.dungeonMask.mapAccesses
      : []),
    ...(Array.isArray(generatedMap?.mapAccesses)
  ...
  sources.forEach((access, index) => {
    if (!access) return;
    const usable =
      access.caveAccessBoundary ||
      access.finalGeometry ||
      access.wallGap ||
...
export function getMapAccessGeometryBasis(access) {
  ...
  if (segment) {
    ...
    if (bounds) {
      ...
      const radial = normalizeGeometryVector(
        { x: center.x - caveCenter.x, y: center.y - caveCenter.y },
        access?.normal || normalA,
      );
      const normal =
    ...
    const fallbackNormal = normalizeGeometryVector(
      access?.normal || getAccessNormalFromSide(access?.side),
      normalA,
    );
    const normal =
      normalA.x * fallbackNormal.x + normalA.y * fallbackNormal.y >= 0
        ? normalA
        : { x: -normalA.x, y: -normalA.y };
    return { normal, tangent };
  }
  const normal = normalizeGeometryVector(
    access?.normal || getAccessNormalFromSide(access?.side),
  );
  const tangent = normalizeGeometryVector(
...
export function smoothClosedContourPoints(
  points,
  iterations = 1,
  strength = 0.5,
) {
...
export function jitterCaveContourPoints(points, seed, amount) {
  ...
  const radialSamples = Array.from(
    { length: sampleCount },
    (_, index) =>
      ((hashStringToSeed(seed, index, "cave-contour-radial") % 1000) / 1000 -
        0.5) *
      amount,
  );
  const tangentSamples = Array.from(
    { length: sampleCount },
    (_, index) =>
      ((hashStringToSeed(seed, index, "cave-contour-tangent") % 1000) / 1000 -
        0.5) *
      amount *
      0.12,
  );
...
export function angularizeCaveContourPoints(
  points,
  seed,
  amount,
  options = {},
  ...
  const tangentAmount = Number.isFinite(options.tangentAmount)
    ? options.tangentAmount
    : amount * 0.48;
  return points.map((point, index) => {
    ...
    const coarse =
      ((hashStringToSeed(seed, Math.floor(index / 2), "angular-coarse") %
        1000) /
        1000 -
        0.5) *
      amount;
    const local =
      ((hashStringToSeed(seed, index, "angular-local") % 1000) / 1000 - 0.5) *
      amount *
      0.38;
    const tangentJitter =
      ((hashStringToSeed(seed, index, "angular-tangent") % 1000) / 1000 - 0.5) *
      tangentAmount;
    const alternating =
      index % preserveEvery === 0
        ? 0
        : (index % 2 === 0 ? -0.18 : 0.18) * amount;
...
export function degridCaveContourPoints(points, seed, amount) {
  ...
  return points.map((point, index) => {
    ...
    const wave =
      Math.sin(index * 1.73 + (hashStringToSeed(seed, "phase") % 628) / 100) *
      amount *
      0.24;
    const normalOffset =
      ((hashStringToSeed(seed, index, "degrid-normal") % 1000) / 1000 - 0.5) *
        amount +
      wave;
    const tangentOffset =
      ((hashStringToSeed(seed, index, "degrid-tangent") % 1000) / 1000 - 0.5) *
      amount *
      0.56;
    return {
...
export function segmentedLoopPathWithSkippedEdges(points, skippedEdges = []) {
  ...
  const skipped = new Set(
    skippedEdges.map((edge) => `${edge.from}:${edge.to}`),
  );
  const paths = [];
  let current = [];
  for (let index = 0; index < points.length; index += 1) {
...
export function getAccessContourPoint(access, config) {
  ...
  if (access?.displayPoint)
    return { x: access.displayPoint.x, y: access.displayPoint.y };
  if (access?.wallGap) {
    return {
      x: (access.wallGap.x1 + access.wallGap.x2) / 2,
      y: (access.wallGap.y1 + access.wallGap.y2) / 2,
...
export function getLoopArcPosition(loop, metrics, edgeIndex, point) {
  ...
  return (
    (metrics.edgeStarts[index] || 0) + distanceBetweenPoints(loop[index], point)
  );
}
...
export function getLoopPointAtDistance(loop, edge, direction, distance) {
  ...
  for (let guard = 0; guard < loop.length + 4; guard += 1) {
    const nextVertexIndex =
      direction < 0 ? edgeIndex : (edgeIndex + 1) % loop.length;
    ...
    edgeIndex =
      direction < 0
        ? (edgeIndex - 1 + loop.length) % loop.length
        : (edgeIndex + 1) % loop.length;
  }
...
export function getCornerSharpness(loop, vertexIndex) {
  ...
  const a = normalizeGeometryVector({
    x: previous.x - current.x,
    y: previous.y - current.y,
  });
  const b = normalizeGeometryVector({
    x: next.x - current.x,
    y: next.y - current.y,
  });
...
export function scoreAccessBoundarySpan(
  loop,
  leftAttach,
  rightAttach,
  ...
  const probeLeft = {
    x: leftAttach.x + normal.x * g * 0.4,
    y: leftAttach.y + normal.y * g * 0.4,
  };
  const probeRight = {
    x: rightAttach.x + normal.x * g * 0.4,
    y: rightAttach.y + normal.y * g * 0.4,
  };
  const insidePenalty =
    (pointInPolygon(probeLeft, loop) ? 40 : 0) +
    (pointInPolygon(probeRight, loop) ? 40 : 0);
...
export function getOutwardNormalForPoint(
  point,
  tangent,
  bounds,
  loop,
  ...
  const probeA = {
    x: point.x + normalA.x * probe,
    y: point.y + normalA.y * probe,
  };
  const probeB = {
    x: point.x + normalB.x * probe,
    y: point.y + normalB.y * probe,
  };
  ...
  const radial = normalizeGeometryVector(
    { x: point.x - caveCenter.x, y: point.y - caveCenter.y },
    normalA,
  );
  const radialNormal =
    normalA.x * radial.x + normalA.y * radial.y >= 0 ? normalA : normalB;
  const radialProbe = {
    x: point.x + radialNormal.x * probe,
    y: point.y + radialNormal.y * probe,
  };
  return pointInPolygon(radialProbe, loop)
    ? { x: -radialNormal.x, y: -radialNormal.y }
    : radialNormal;
}
...
export function createAccessMouthForLoop(
  access,
  edge,
  loop,
  config,
  ...
  const tangent = normalizeGeometryVector(
    { x: dx, y: dy },
    access.tangent || { x: 1, y: 0 },
  );
  const normal = getOutwardNormalForPoint(
    edge.projected,
    tangent,
    bounds,
    loop,
  ...
  const jitterSeed = hashStringToSeed(
    config.seed,
    access.id || order,
    "access-mouth",
  );
  const jitter = (part, amount) =>
    ((hashStringToSeed(jitterSeed, part) % 1000) / 1000 - 0.5) * amount;
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    const removed = collectLoopVertexRange(
      loop,
      leftCandidate.edgeIndex,
      rightCandidate.edgeIndex,
    );
    const spanScore = scoreAccessBoundarySpan(
      loop,
      leftCandidate,
      rightCandidate,
      removed,
  ...
  const mouthVector = normalizeGeometryVector(
    { x: rightAttach.x - leftAttach.x, y: rightAttach.y - leftAttach.y },
    tangent,
  );
  const mouthSpan = Math.max(
    g * 0.85,
    distanceBetweenPoints(leftAttach, rightAttach),
  );
  const mouthHalfActual = mouthSpan / 2;
  const tipHalf =
    mouthHalf *
    (0.48 + ((hashStringToSeed(jitterSeed, "tip") % 100) / 100) * 0.16);
  const shoulderHalf = Math.min(mouthHalfActual * 0.72, g * 0.86);
  const depth =
    g * (0.92 + ((hashStringToSeed(jitterSeed, "depth") % 100) / 100) * 0.3);
  ...
  let startS = getLoopArcPosition(
    loop,
    metrics,
    leftAttach.edgeIndex,
    leftMouth,
  );
  let endS = getLoopArcPosition(
    loop,
    metrics,
    rightAttach.edgeIndex,
    rightMouth,
  );
  if (endS < startS) endS += metrics.totalLength;
  const projectedCenter = { x: projected.x, y: projected.y };
  const centerProjection = projectPointToLineSegment(
    projectedCenter,
    leftMouth,
    rightMouth,
  );
  ...
  const leftShoulderOuter = {
    x:
      shoulderCenter.x -
      mouthVector.x * shoulderHalf +
      normal.x * jitter("left-shoulder-n", g * 0.035) +
      mouthVector.x * jitter("left-shoulder-t", g * 0.04),
    y:
      shoulderCenter.y -
      mouthVector.y * shoulderHalf +
      normal.y * jitter("left-shoulder-n", g * 0.035) +
      mouthVector.y * jitter("left-shoulder-t", g * 0.04),
  };
  const leftTip = {
    x:
      tipCenter.x -
      mouthVector.x * tipHalf +
      normal.x * jitter("left-tip-n", g * 0.025) +
      mouthVector.x * jitter("left-tip-t", g * 0.03),
    y:
      tipCenter.y -
      mouthVector.y * tipHalf +
      normal.y * jitter("left-tip-n", g * 0.025) +
      mouthVector.y * jitter("left-tip-t", g * 0.03),
  };
  const rightTip = {
    x:
      tipCenter.x +
      mouthVector.x * tipHalf +
      normal.x * jitter("right-tip-n", g * 0.025) +
      mouthVector.x * jitter("right-tip-t", g * 0.03),
    y:
      tipCenter.y +
      mouthVector.y * tipHalf +
      normal.y * jitter("right-tip-n", g * 0.025) +
      mouthVector.y * jitter("right-tip-t", g * 0.03),
  };
  const rightShoulderOuter = {
    x:
      shoulderCenter.x +
      mouthVector.x * shoulderHalf +
      normal.x * jitter("right-shoulder-n", g * 0.035) +
      mouthVector.x * jitter("right-shoulder-t", g * 0.04),
    y:
      shoulderCenter.y +
      mouthVector.y * shoulderHalf +
      normal.y * jitter("right-shoulder-n", g * 0.035) +
      mouthVector.y * jitter("right-shoulder-t", g * 0.04),
  };
  return {
    ...
    displayPoint: center,
    handlePoint: {
      x: center.x + normal.x * g * 0.16,
      y: center.y + normal.y * g * 0.16,
    },
    start: {
      x: center.x + normal.x * (depth + g * 0.1),
      y: center.y + normal.y * (depth + g * 0.1),
    },
    end: { x: center.x - normal.x * g * 0.3, y: center.y - normal.y * g * 0.3 },
    caveBounds: bounds,
    debugRequestedPoint:
      access.point || access.displayPoint || access.start || null,
...
export function applyCaveAccessMouthsToBoundaryLoop(loop, accesses, config) {
  ...
  const mouths = usableAccesses
    .map((access, order) => {
      ...
      const mouth = createAccessMouthForLoop(
        access,
        edge,
        loop,
        config,
  ...
  mouths.forEach((mouth) => {
    const overlapsExisting = stableMouths.some(
      (existing) =>
        mouth.startS < existing.endS + minSpacing &&
        mouth.endS + minSpacing > existing.startS,
    );
  ...
  stableMouths.forEach((mouth) => {
    while (
      cursorIndex < loop.length &&
      (metrics.edgeStarts[cursorIndex] || 0) < mouth.startS
    ) {
    ...
    while (
      cursorIndex < loop.length &&
      (metrics.edgeStarts[cursorIndex] || 0) <= mouth.endS
    ) {
      cursorIndex += 1;
    }
    const mouthSkippedEdges = getSkippedWallEdgesForMouth(mouth, insertedStart);
    mouth.skippedEdges = mouthSkippedEdges;
    mouth.drawEdges = (mouth.edgeRoles || []).filter(
      (edge) => edge.drawWall !== false,
    );
    mouth.removedStartIndex = mouth.removedVertexIndices?.[0] ?? null;
    mouth.removedEndIndex =
      mouth.removedVertexIndices?.[mouth.removedVertexIndices.length - 1] ??
      null;
...
export function removeMouthCoveredBoundaryPoints(points, mouth, insertedStart) {
  if (
    !mouth ||
    !Array.isArray(mouth.removedVertexIndices) ||
    mouth.removedVertexIndices.length === 0
  )
    return points;
  ...
  return points.filter((point, index) => {
    ...
    const originalIndex =
      index > insertedEnd ? index - mouth.points.length : index;
    return !removed.has(originalIndex);
  });
}

export function createHexCaveContourPointsFromSegments(
  segments,
  config,
  layer = "floor",
) {
  ...
  const thinned = thinClosedContourPoints(
    softenedGrid,
    config.gridSize * (layer === "wall" ? 0.58 : 0.52),
  );
  const angularPoints = densifyAngularContourPoints(
    thinned,
    config.gridSize * (layer === "wall" ? 0.54 : 0.48),
  );
  const degridded = degridCaveContourPoints(
    angularPoints,
    `${seed}:0:degrid`,
    config.gridSize * (layer === "floor" ? 0.18 : 0.13),
  );
  const jittered = angularizeCaveContourPoints(
    degridded,
    `${seed}:0`,
    config.gridSize * (layer === "floor" ? 0.14 : 0.1),
    { preserveEvery: 3, tangentAmount: config.gridSize * 0.14 },
  );
...
export function createHexCavePathFromSegments(
  ...
  return segmentedClosedPath(
    createHexCaveContourPointsFromSegments(segments, config, layer),
  );
}
...
export function createHexCaveSurface(generatedMap) {
  ...
  const accesses = getCaveMapAccessesForGeometry(generatedMap);
  const baseFloorLoop = createHexCaveContourPointsFromSegments(
    boundarySegments,
    generatedMap.config,
    "floor",
  );
  const baseWallLoop = createHexCaveContourPointsFromSegments(
    boundarySegments,
    generatedMap.config,
    "wall",
  );
  const baseSketchLoop = createHexCaveContourPointsFromSegments(
    boundarySegments,
    generatedMap.config,
    "sketch",
  );
  const floorResult = applyCaveAccessMouthsToBoundaryLoop(
    baseFloorLoop,
    accesses,
    generatedMap.config,
  );
  const wallResult = applyCaveAccessMouthsToBoundaryLoop(
    baseWallLoop,
    accesses,
    generatedMap.config,
  );
  const sketchResult = applyCaveAccessMouthsToBoundaryLoop(
    baseSketchLoop,
    accesses,
    generatedMap.config,
  );
  const visualFloorPath = segmentedClosedPath(floorResult.points);
  const wallPath = segmentedLoopPathWithSkippedEdges(
    wallResult.points,
    wallResult.skippedEdges,
  );
  const sketchPath = segmentedLoopPathWithSkippedEdges(
    sketchResult.points,
    sketchResult.skippedEdges,
  );
  const accessBoundarySegments =
    wallResult.points.length > 0
      ? loopToSegments(wallResult.points, wallResult.skippedEdges)
      : boundarySegments;
...
export function buildOrganicCaveContourPoints(floorCells, gridSize, seed) {
  ...
  const jittered = angularizeCaveContourPoints(
    ...
    `${seed}:loop:0`,
    gridSize * 0.13,
    { preserveEvery: 3, tangentAmount: gridSize * 0.13 },
  );
  return smoothClosedContourPoints(jittered, 1, 0.008);
}
...
export function buildOrganicCaveContourPath(floorCells, gridSize, seed) {
  return segmentedClosedPath(
    buildOrganicCaveContourPoints(floorCells, gridSize, seed),
  );
}
...
export function createCellBasedCaveSurface(generatedMap) {
  ...
  const baseContour = buildOrganicCaveContourPoints(
    renderCells,
    config.gridSize,
    hashStringToSeed(config.seed, "cell-cave-unified-contour"),
  );
  const accessResult = applyCaveAccessMouthsToBoundaryLoop(
    baseContour,
    getCaveMapAccessesForGeometry(generatedMap),
    config,
  );
  const visualFloorPath =
    segmentedClosedPath(accessResult.points) ||
    buildFloorPath(renderCells, config.gridSize);
  const wallPath =
    segmentedLoopPathWithSkippedEdges(
      accessResult.points,
      accessResult.skippedEdges,
    ) || visualFloorPath;
  const accessBoundarySegments =
    accessResult.points.length > 0
      ? loopToSegments(accessResult.points, accessResult.skippedEdges)
      : boundarySegments;
  return {
...
export function buildOrganicCellBoundaryPath(
  ...
  const floorCells = isSingleRegionCaveMap(generatedMap)
    ? createNaturalizedSingleRegionCells(sourceCells, {
...
export function createMineCaveConnectorPad(
  corridor,
  endpoint,
  region,
  gridSize,
  ...
  const corridorTerminalLeft = {
    x: corridorTerminalCenter.x - tangent.x * half,
    y: corridorTerminalCenter.y - tangent.y * half,
  };
  const corridorTerminalRight = {
    x: corridorTerminalCenter.x + tangent.x * half,
    y: corridorTerminalCenter.y + tangent.y * half,
  };
  const caveAttachLeft = {
    x: caveAttachCenter.x - tangent.x * half,
    y: caveAttachCenter.y - tangent.y * half,
  };
  const caveAttachRight = {
    x: caveAttachCenter.x + tangent.x * half,
    y: caveAttachCenter.y + tangent.y * half,
  };
  return {
    ...
    openToCave: true,
    corridorOpenSegment: {
      x1: corridorTerminalLeft.x,
      y1: corridorTerminalLeft.y,
      x2: corridorTerminalRight.x,
      y2: corridorTerminalRight.y,
    },
  };
}
...
export function collectMineCaveConnectorPads(generatedMap, caveRegionIds) {
  ...
  const addPad = (corridor, endpoint) => {
    ...
    if (!region) return;
    const pad = createMineCaveConnectorPad(
      corridor,
      endpoint,
      region,
      gridSize,
    ...
    if (
      !existing.some(
        (item) =>
          item.corridorId === pad.corridorId && item.endpoint === pad.endpoint,
      )
    )
      existing.push(pad);
  };
...
export function addMineCaveConnectorPadsToRegion(region, connectorPads) {
  if (!Array.isArray(connectorPads) || connectorPads.length === 0)
    return region;
  const cellsByKey = new Map(
    (region.floorCells || []).map((cell) => [
      cellKey(cell.x, cell.y),
      { x: cell.x, y: cell.y },
    ]),
  );
  connectorPads.forEach((pad) => {
    (pad.cells || []).forEach((cell) =>
...
export function isMineCaveExternalAccess(access) {
  return ["entrance", "exit", "passage", "in", "out", "pass"].includes(
    access?.type,
  );
}
...
export function collectMineCaveMapAccesses(generatedMap, caveRegionIds) {
  ...
  (
    ...
    if (
      !access?.regionId ||
      !caveRegionIds.has(access.regionId) ||
      !isMineCaveExternalAccess(access)
    )
      return;
    if (!accessesByRegion.has(access.regionId))
      accessesByRegion.set(access.regionId, []);
...
export function createMineCaveAccessMouthFromLoopMouth(
  mouth,
  access,
  regionId,
) {
...
export function getMinePassMouthInsertedStart(mouth) {
  const outerSkip = (mouth?.skippedEdges || []).find(
    (edge) => edge.role === "outer-open-edge",
  );
  return Number.isFinite(outerSkip?.from) ? outerSkip.from - 2 : -1;
}

export function alignMineCavePassMouthsToCorridorSeams(
  accessResult,
  passAccesses,
) {
  if (!accessResult?.points?.length || !accessResult?.mouths?.length)
    return accessResult;
  const accessById = new Map(
    (passAccesses || []).map((access) => [access.id, access]),
  );
  accessResult.mouths.forEach((mouth) => {
    const connectorPad = accessById.get(mouth.accessId)?.connectorPad;
    if (
      !connectorPad?.corridorTerminalLeft ||
      !connectorPad?.corridorTerminalRight
    )
      return;
    const insertedStart = getMinePassMouthInsertedStart(mouth);
    if (insertedStart < 0 || insertedStart + 4 >= accessResult.points.length)
      return;
    const terminalLeft = {
      x: connectorPad.corridorTerminalLeft.x,
      y: connectorPad.corridorTerminalLeft.y,
    };
    const terminalRight = {
      x: connectorPad.corridorTerminalRight.x,
      y: connectorPad.corridorTerminalRight.y,
    };
    ...
    const mouthTangent =
      terminalVector.x * mouth.tangent.x + terminalVector.y * mouth.tangent.y >=
      0
        ? terminalVector
        : { x: -terminalVector.x, y: -terminalVector.y };
    const corridorWidth = Math.max(
      distanceBetweenPoints(terminalLeft, terminalRight),
      connectorPad.width || 0,
    );
    const mouthHalf = Math.max(
      corridorWidth * 0.55,
      Math.min(
        corridorWidth * 0.675,
        connectorPad.width * 0.675 || corridorWidth * 0.625,
      ),
    );
    const existingDepth = distanceBetweenPoints(mouth.center, terminalCenter);
    const desiredDepth = Math.max(
      connectorPad.depth * 0.65,
      Math.min(connectorPad.depth, existingDepth || connectorPad.depth * 0.78),
    );
    const mouthCenter = {
    ...
    mouth.seam = {
      ...
      depth: distanceBetweenPoints(
        mouthCenter,
        connectorPad.corridorTerminalCenter,
      ),
    };
  });
  return accessResult;
...
export function createFallbackMineCaveContourFromCells(
  floorCells,
  gridSize,
  seed,
) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const noise =
      ((hashStringToSeed(seed, index, "mine-cave-fallback-contour") % 1000) /
        1000 -
        0.5) *
      gridSize *
      0.18;
    const tangentNoise =
      ((hashStringToSeed(seed, index, "mine-cave-fallback-tangent") % 1000) /
        1000 -
        0.5) *
      gridSize *
      0.1;
    return {
      x:
        cx +
        Math.cos(angle) * (rx + noise) +
        Math.cos(angle + Math.PI / 2) * tangentNoise,
      y:
        cy +
        Math.sin(angle) * (ry + noise) +
        Math.sin(angle + Math.PI / 2) * tangentNoise,
    };
  });
}

export function createMineHybridOrganicRegionSurface(
  region,
  generatedMap,
  connectorPads,
  mapAccesses = [],
  ...
  let baseContour = buildOrganicCaveContourPoints(
    contourCells,
    gridSize,
    hashStringToSeed(
      seed,
  ...
  if (!Array.isArray(baseContour) || baseContour.length < 3) {
    contourCells = createNaturalCaveVisualCells(floorCells, {
      ...generatedMap.config,
      gridSize,
    });
    baseContour = buildOrganicCaveContourPoints(
      contourCells,
      gridSize,
      hashStringToSeed(
        seed,
        region.id,
        region.shape || "cave",
        "organic-region-contour-fallback",
      ),
    );
  }
  if (!Array.isArray(baseContour) || baseContour.length < 3) {
    ...
    baseContour = buildOrganicCaveContourPoints(
      ...
      hashStringToSeed(
        ...
        region.id,
        region.shape || "cave",
        "organic-region-contour-expanded",
      ),
    );
  }
  if (!Array.isArray(baseContour) || baseContour.length < 3) {
    baseContour = createFallbackMineCaveContourFromCells(
      floorCells,
      gridSize,
      hashStringToSeed(
        seed,
        region.id,
        region.shape || "cave",
        "organic-region-contour-local-fallback",
      ),
    );
  }
  ...
  const accessResult = alignMineCavePassMouthsToCorridorSeams(
    applyCaveAccessMouthsToBoundaryLoop(
      baseContour,
      allMouthInputs,
      generatedMap.config,
    ),
  ...
  const passMouths = (accessResult.mouths || [])
    .map((mouth) => {
      const connectorPad =
        passAccesses.find((access) => access.id === mouth.accessId)
          ?.connectorPad || null;
    ...
    .filter(Boolean);
  const externalAccessById = new Map(
    externalAccesses.map((access) => [access.id, access.mapAccess]),
  );
  const accessMouths = (accessResult.mouths || [])
    .map((mouth) =>
      createMineCaveAccessMouthFromLoopMouth(
        mouth,
        externalAccessById.get(mouth.accessId),
        region.id,
      ),
    )
    .filter(Boolean);
...
export function finalizeHybridGeometry(generatedMap) {
  ...
  const caveRegions = (generatedMap?.regions || []).filter((region) =>
    isHybridLocalCaveRegion(region, generatedMap),
  );
  if (caveRegions.length === 0) return null;
  const caveRegionIds = new Set(caveRegions.map((region) => region.id));
  const connectorPadsByRegion = collectMineCaveConnectorPads(
    generatedMap,
    caveRegionIds,
  );
  const mapAccessesByRegion = collectMineCaveMapAccesses(
    generatedMap,
    caveRegionIds,
  );
  const regions = Object.fromEntries(
    caveRegions.map((region) => {
      const connectorPads = connectorPadsByRegion.get(region.id) || [];
      const mapAccesses = mapAccessesByRegion.get(region.id) || [];
      const surface =
        createMineHybridOrganicRegionSurface(
          region,
          generatedMap,
          connectorPads,
          mapAccesses,
      ...
      const wallSegments =
        surface.wallSegments || surface.boundarySegments || [];
      const organicSurface =
        surface.geometryKind === "organic-cell-mask" &&
        isUsableSvgPath(
          surface.wallPath || surface.wallArcPath || surface.visualFloorPath,
        );
      const fallbackWallPath =
        buildBoundarySegmentPath(wallSegments) ||
        buildBoundarySegmentPath(surface.boundarySegments);
      ...
      return [
        region.id,
        {
          ...
          finalGeometry: true,
          surfaceKind: "cave",
          geometryQuality: organicSurface
            ? "organic"
            : "degraded-boundary-fallback",
          wallPath,
          sketchPath,
          wallSegments,
  ...
  return (
    segments
      .map((segment, index) => {
        ...
        const dy = projected.y - point.y;
        const length =
          Math.hypot(segment.x2 - segment.x1, segment.y2 - segment.y1) || 1;
        return {
          index,
          segment,
          point: projected,
          tangent: {
            x: (segment.x2 - segment.x1) / length,
            y: (segment.y2 - segment.y1) / length,
          },
          score: dx * dx + dy * dy,
        };
      })
...
export function createHybridBreachMouth(
  door,
  surface,
  region,
  corridorId = door?.corridorId,
) {
  if (!door || !isHybridBreachEndpoint(region, surface)) return null;
  const projection = getClosestHybridBoundaryProjection(
    getDoorCenter(door),
    surface,
  );
  ...
  const towardCorridor = {
    x: doorCenter.x - center.x,
    y: doorCenter.y - center.y,
  };
  const normal =
    outward.x * towardCorridor.x + outward.y * towardCorridor.y >= 0
      ? outward
      : { x: -outward.x, y: -outward.y };
  const projectedDepth =
    normal.x * towardCorridor.x + normal.y * towardCorridor.y;
  const depth = Math.max(
    gridSize * 0.55,
    Math.min(gridSize * 1.35, projectedDepth || gridSize * 0.72),
  );
  const leftAttach = {
  ...
  const innerCenter = {
    x: center.x - normal.x * gridSize * 0.16,
    y: center.y - normal.y * gridSize * 0.16,
  };
  const outerCenter = {
    x: center.x + normal.x * depth,
    y: center.y + normal.y * depth,
  ...
  const terminalLeft = {
    x: terminalCenter.x - tangent.x * terminalHalf,
    y: terminalCenter.y - tangent.y * terminalHalf,
  };
  const terminalRight = {
    x: terminalCenter.x + tangent.x * terminalHalf,
    y: terminalCenter.y + tangent.y * terminalHalf,
...
export function createHybridBreachDoor(door, surface, region) {
  const mouth = createHybridBreachMouth(
    door,
    surface,
    region,
    door?.corridorId,
...
export function createHybridEndpointOpening(
  corridor,
  endpoint,
  region,
  surface,
  ...
  if (isHybridBreachEndpoint(region, surface)) {
    const caveConnectorPad =
      (surface.connectorPads || []).find(
        (pad) => pad.corridorId === corridor.id && pad.endpoint === endpoint,
      ) || null;
    return {
      ...base,
...
export function reconcileHybridCorridorAnchors(generatedMap, finalGeometry) {
  if (!finalGeometry?.regions || isPureCaveMap(generatedMap))
  ...
  return (generatedMap.corridors || []).map((corridor) => {
    ...
    const fromDoor =
      (corridor.doors || []).find(
        (door) => door.endpoint === "from" || door.endpoint === "shared",
      ) || null;
    const toDoor =
      (corridor.doors || []).find(
        (door) => door.endpoint === "to" || door.endpoint === "shared",
      ) || null;
    const fromOpening = createHybridEndpointOpening(
      corridor,
      "from",
      fromRegion,
      fromRegion ? finalGeometry.regions[fromRegion.id] : null,
      fromDoor,
    );
    const toOpening = createHybridEndpointOpening(
      corridor,
      "to",
      toRegion,
      toRegion ? finalGeometry.regions[toRegion.id] : null,
      toDoor,
    );
    const doors = (corridor.doors || []).map((door) => {
      if (door.endpoint === "from")
        return fromOpening.openingType === "breach"
          ? createHybridConnectorDoor(door, fromOpening.caveConnectorPad)
          : stripHybridBreachDoorFields(door);
      if (door.endpoint === "to")
        return toOpening.openingType === "breach"
          ? createHybridConnectorDoor(door, toOpening.caveConnectorPad)
          : stripHybridBreachDoorFields(door);
      if (door.endpoint === "shared") {
        if (fromOpening.openingType === "breach")
          return createHybridConnectorDoor(door, fromOpening.caveConnectorPad);
        if (toOpening.openingType === "breach")
          return createHybridConnectorDoor(door, toOpening.caveConnectorPad);
      }
...
export function createFinalCaveRegionSurface(
  ...
  const baseSurface = createCellMaskRegionSurface(
    region,
    generatedMap,
    generatedMap.config.gridSize,
  );
