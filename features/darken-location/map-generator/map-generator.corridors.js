...
export function getSharedRoomConnection(from, to, gridSize, rng, manualFromAnchor = null, manualToAnchor = null, profile = {}) {
  const connections = getSharedBoundaryConnections(from, to, gridSize);
  if (connections.length === 0) return null;
...
export function buildOrganicTunnelFloorCells(path, config, rng, blockedRoomCells = new Set(), corridorId = "organic-corridor") {
  const contextKey = getContextKey(config.context || config.biome);
...
export function resolveCorridorDrawLevel(corridor, fromLevel, toLevel, transition) {
...
export function getCorridorCrossingOrientation(corridor, cell, fallback = "horizontal") {
