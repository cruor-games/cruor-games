...
import { HORROR_TYPES, INTRUSION_LEVELS, INTRUSION_LABELS, HORROR_DESCRIPTIONS, CONTEXT_DESCRIPTIONS } from './crucible.brief-options.js';
import { SOURCE_ANCHORS, SOURCE_TYPES, THEMES, SOURCE_DETAILS, INSPIRATION_CARDS } from './crucible.sources-data.js';
...
import { CRUOR_COMPOSER_NAVIGATOR_WIDTH_KEY, CRUOR_COMPOSER_SAVES_KEY } from './crucible.storage.js';

export function startCrucibleDomApp(rootElement = document, options = {}) {
  const onOpenMapGenerator = typeof options.onOpenMapGenerator === 'function' ? options.onOpenMapGenerator : null;
  const onSnapshotProviderReady = typeof options.onSnapshotProviderReady === 'function' ? options.onSnapshotProviderReady : null;
  ...
  function runRuntimeTests() {
    ...
    console.assert(publicAnchors.every(anchor => SOURCE_DETAILS[anchor]), 'Every Source Anchor must define SOURCE_DETAILS.');
    console.assert(publicAnchors.every(anchor => INSPIRATION_CARDS.some(card => card.anchor === anchor)), 'Every Source Anchor must have an Inspiration card.');
    ...
    console.assert(SOURCE_DETAILS['Towers of Silence'], 'SOURCE_DETAILS must define Towers of Silence.');
    console.assert(SOURCE_DETAILS['Anthropodermic Bibliopegy'], 'SOURCE_DETAILS must define Anthropodermic Bibliopegy.');
    console.assert(SOURCE_DETAILS.Decomposition, 'SOURCE_DETAILS must define Decomposition.');
    console.assert(!String(document.documentElement.innerHTML).includes('{$1}'), 'Template placeholder {$1} must not exist in the document.');
    console.assert(!String(document.documentElement.innerHTML).includes('$1backdrop-filter'), 'CSS regex backreference fragments must never be written into the stylesheet.');
    console.assert(COMPONENTS.every(component => Array.isArray(component.sourceAnchors)), 'Every component must have sourceAnchors array.');
    console.assert(COMPONENTS.every(component => Array.isArray(component.sourceTypes)), 'Every component must have sourceTypes array.');
    console.assert(COMPONENTS.every(component => Array.isArray(component.themes)), 'Every component must have themes array.');
    console.assert(COMPONENTS.every(component => component.sourceAnchors.every(anchor => SOURCE_DETAILS[anchor])), 'Every component Source Anchor must exist in SOURCE_DETAILS.');
    console.assert(publicAnchors.every(anchor => COMPONENTS.some(component => component.sourceAnchors.includes(anchor))), 'Every public Source Anchor should have at least one linked component in the MVP seed data.');
    console.assert(Object.values(WORKFLOWS).every(workflow => workflow.description), 'Every workflow must provide a short description.');
    console.assert(els.workflowButtons && !document.querySelector('#needSelect'), 'Workflow selection must use icon toggle buttons, not a select dropdown.');
    console.assert(!document.querySelector('[data-view="compose"]') && !document.querySelector('[data-view="table"]'), 'Compose and Table View buttons must not be rendered.');
    ...
    console.assert(!document.getElementById('workflowDescription'), 'Workflow description must not be rendered.');
    console.assert(!document.querySelector('.brief-head .eyebrow'), 'Build Brief eyebrow must not be rendered.');
    console.assert(!document.documentElement.innerHTML.includes('.brief-panel') && !document.documentElement.innerHTML.includes('.pipeline-field') && !document.documentElement.innerHTML.includes('.brief-combobox'), 'Legacy Brief Panel, pipeline, and combobox CSS must be removed after the Wizard migration.');
    console.assert(els.tagFilterBtn && els.tagFilterRow, 'Tag filters must be controlled by a compact contextual menu.');
    console.assert(!renderEmptySlot({ id: 'test', label: 'Test Slot' }).includes('<button'), 'Empty slots must not render a redundant add button.');
    console.assert(typeof renderCompiledPreview === 'function', 'Compiled preview must use the custom editorial renderer.');
    console.assert(state.tags instanceof Set, 'Tag filters must support multi-select state.');
    console.assert(state.sourceAnchors instanceof Set, 'Source Anchor filters must support multi-select state.');
    console.assert(els.briefWizardModal && els.sourcePickerModal, 'Draw from must use the Brief Wizard / Inspiration Picker flow, not a static select.');
    console.assert(!document.querySelector('#sourceSelect'), 'Legacy Draw from select must not be rendered.');
    console.assert(!document.querySelector('#sourceTypeFilter') && !document.querySelector('#sourceThemeFilter'), 'Source Type and Theme filters must use searchable combobox controls, not native selects.');
    console.assert(SOURCE_TYPES.slice(1).every((value, index, array) => index === 0 || array[index - 1].localeCompare(value) <= 0), 'Source Types must be sorted alphabetically after Any Type.');
    console.assert(THEMES.every((value, index, array) => index === 0 || array[index - 1].localeCompare(value) <= 0), 'Themes must be sorted alphabetically.');
    console.assert(displayTitle('The Cave Has Been Breathing') === 'The cave has been breathing', 'displayTitle must produce readable sentence-style UI titles.');
    console.assert(typeof refreshNavigatorSelectionState === 'function', 'Navigator card selection must update without rerendering the whole list.');
    console.assert(state.activeSlot === 'horrorPremise', 'Default active location slot must be Premise.');
    console.assert(WORKFLOWS.location.slots.some(slot => slot.label === 'Outcome'), 'Location workflow must use Outcome as the final slot label.');
    console.assert(!WORKFLOWS.location.slots.some(slot => /Reward or Consequence|Disturbing Clue/.test(slot.label)), 'Location workflow must not expose outdated verbose slot labels.');
    console.assert(renderEmptySlot({ id: 'horrorPremise', label: 'Premise' }).includes('Choose the core horror idea'), 'Empty slot copy must explain the practical slot function.');
    console.assert(typeof getMatchReasons === 'function', 'Navigator cards must explain why components match current filters.');
    console.assert(typeof openInspirationDetail === 'function', 'Inspiration cards must support a public detail view.');
    console.assert(COMPONENTS.filter(component => component.sourceAnchors.includes('Decomposition') && component.workflows.includes('location')).length >= 9, 'Decomposition should have a complete Location source pack in the MVP seed data.');
    ['Decomposition', 'Sedlec Ossuary', 'Towers of Silence', 'The Mist', 'Jikininki', 'Gashadokuro'].forEach(anchor => {
      const pack = COMPONENTS.filter(component => component.sourceAnchors.includes(anchor) && component.workflows.includes('location'));
      const hasSlot = slotId => pack.some(component => component.slots.includes(slotId));
      console.assert(pack.length >= 9, `${anchor} Location source pack should include at least 9 components.`);
      console.assert(hasSlot('horrorPremise'), `${anchor} pack must include a Premise.`);
      console.assert(hasSlot('sensoryLayer'), `${anchor} pack must include Sensory Layer components.`);
      console.assert(['Sound', 'Smell', 'Touch'].every(kind => pack.some(component => component.slots.includes('sensoryLayer') && component.sensoryKind === kind)), `${anchor} pack must include Sound, Smell, and Touch.`);
    ...
    console.assert(HORROR_TYPES.includes('War Horror'), 'HORROR_TYPES must include War Horror because seed data uses it.');
    console.assert(COMPONENTS.filter(component => component.slots.includes('sensoryLayer')).every(component => SENSORY_KINDS.includes(component.sensoryKind)), 'Every Sensory Layer component must define a valid sensoryKind.');
    console.assert(typeof pickUniqueForSlot === 'function', 'Slot regeneration must avoid duplicate Sensory Layer kinds.');
    console.assert(typeof showLinkedComponent === 'function', 'Inspiration linked components must navigate to a visible component state.');
    console.assert(!document.querySelector('#contextSelect') && !document.querySelector('#horrorSelect') && !document.querySelector('#intrusionSelect'), 'Brief controls must use custom controls, not native browser selects.');
    console.assert(els.briefSummaryText && els.editBriefBtn && els.briefWizardModal, 'Brief must render as a compact summary with a guided wizard.');
    console.assert(!document.querySelector('.brief-panel'), 'The persistent Brief Panel must not be rendered in the main layout.');
    console.assert(!document.documentElement.innerHTML.includes('.sensory-subslot:not(.filled)::before'), 'Empty border animation must not be applied to sensory subslots.');
    console.assert(document.documentElement.innerHTML.includes('.build-slot.needs-attention:not(.active)::before'), 'Empty border animation must skip the active slot and target only attention-needed main slots.');
    console.assert(document.documentElement.innerHTML.includes('inset: 1px;'), 'Empty border animation must expose only a 1px animated border.');
    console.assert(typeof updateActiveSlotUi === 'function', 'Selecting a slot must update active classes without rerendering all slots, so empty-slot animations do not reset.');
    console.assert(typeof clearActiveSlot === 'function' && typeof handleBuildCanvasEmptyClick === 'function' && document.documentElement.innerHTML.includes('els.buildCanvas.addEventListener'), 'Clicking empty space inside the build canvas must deselect the active slot without reacting to clicks outside the canvas.');
    console.assert(getComputedStyle(document.documentElement).getPropertyValue('--glass-satin-strong').includes('29, 23, 25'), 'Main containers must use a darker satin surface than v0.62 while still letting dark slots read as inset wells.');
    console.assert(document.documentElement.innerHTML.includes('has-items') && document.documentElement.innerHTML.includes('complete'), 'Slot rendering must distinguish has-items from complete so incomplete multi-slots keep the incomplete background.');
    console.assert(!renderSlotItem('sensoryLayer', { id: 'test-sound', title: 'Test Sound', summary: 'Test summary.', sensoryKind: 'Sound' }, 1, { hideSensoryKind: true }).includes('slot-item-kind'), 'Sensory subslots must not repeat the Sound/Smell/Touch pill inside the selected component title.');
    console.assert(document.documentElement.innerHTML.includes('.build-slot.active .slot-icon') && document.documentElement.innerHTML.includes('color: #d0a2aa'), 'Active slot icons must use the same bright color as active slot titles.');
    console.assert(document.documentElement.innerHTML.includes('.sensory-subslot.active .sensory-subslot__label i'), 'Active sensory subslot labels and icons must use the brighter active color.');
    console.assert(document.documentElement.innerHTML.includes('animation: emptySlotBorderOrbit 8.3s linear infinite'), 'Empty slot border animation must be slowed by 100%.');
    console.assert(document.documentElement.innerHTML.includes('opacity: .62'), 'Empty slot border animation opacity must be reduced by about one quarter.');
    console.assert(!document.documentElement.innerHTML.includes('.brief-wizard-guide') && typeof applyBriefWizard === 'function' && typeof fillCrucibleFromWizard === 'function', 'Brief Wizard must rely on the subtitle for guidance and expose Apply Brief plus Fill the Crucible actions.');
    console.assert(document.documentElement.innerHTML.includes('.brief-wizard__body.is-fading') && typeof transitionBriefWizardTo === 'function', 'Brief Wizard step changes must fade out and fade in instead of changing abruptly.');
    console.assert(document.documentElement.innerHTML.includes('.navigator-head h2') && document.documentElement.innerHTML.includes('.search-wrap::before'), 'Navigator title and search bar must keep styled custom UI after removing legacy Brief Panel CSS.');
    console.assert(document.documentElement.innerHTML.includes('.brief-choice-card.selected') && document.documentElement.innerHTML.includes('.brief-choice-icon i') && document.documentElement.innerHTML.includes('font-size: 72px') && document.documentElement.innerHTML.includes('right: 14px;') && document.documentElement.innerHTML.includes('z-index: 0;'), 'Wizard choice cards must use Crucible-style selected states and right-aligned background watermark icons that do not move the text.');
    console.assert(document.documentElement.innerHTML.includes('.brief-wizard-source-card') && document.documentElement.innerHTML.includes('opacity: .75') && document.documentElement.innerHTML.includes('font-size: 12px') && document.documentElement.innerHTML.includes('font-weight: 650') && document.documentElement.innerHTML.includes('filter: none'), 'Wizard inspiration cards must match the Component Navigator visual language, dim unselected cards, and restore selected images without filters.');
    console.assert(document.documentElement.innerHTML.includes('.brief-wizard-source-card .source-card-visual') && document.documentElement.innerHTML.includes('backdrop-filter: blur(2px) saturate(118%)') && document.documentElement.innerHTML.includes('box-shadow: 0 -18px 38px rgba(0, 0, 0, .5)'), 'Wizard Draw From cards must use the same image-under-text overlay style as Inspiration cards.');
    console.assert(document.documentElement.innerHTML.indexOf('id="briefWizardApply"') < document.documentElement.innerHTML.indexOf('id="briefWizardFill"') && document.documentElement.innerHTML.includes('Apply and fill the Crucible') && document.documentElement.innerHTML.includes('els.briefWizardFill.hidden = state.briefWizardStep !== 3'), 'Wizard final actions must show Apply first and Apply and fill the Crucible as the rightmost final button.');
    console.assert(formatMetaValue('height', 'title') === 'Height' && formatMetaValue('sun warmed bone', 'title') === 'Sun Warmed Bone' && formatManualTitle('the corpse refused by the sky') === 'The Corpse Refused by the Sky', 'Motif and slot title case must capitalize major words while preserving minor words inside titles.');
    console.assert(renderSlotItem('clue', { id: 'test', title: 'the corpse refused by the sky', summary: 'One corpse remains untouched.' }, 1).includes('<strong class="slot-item-title">The Corpse Refused by the Sky.</strong> One corpse remains untouched.'), 'Slot items must render as D&D-style inline title-plus-description prose.');
    console.assert(document.documentElement.innerHTML.includes('.sensory-subslot.filled') && document.documentElement.innerHTML.includes('background-blend-mode: screen, screen, normal'), 'Filled sensory subslots must use the same visual language as build slots.');
    console.assert(document.documentElement.innerHTML.includes('.slot-item {') && document.documentElement.innerHTML.includes('background: transparent;') && document.documentElement.innerHTML.includes('box-shadow: none;'), 'Slot items must not add nested card backgrounds inside build slots.');
    console.assert(['Crucifixion', 'Decomposition', 'Genetic Mutations', 'Wax Death Masks', 'The Mist', 'Sedlec Ossuary', 'Endocannibalism', 'Jikininki', 'Impalement', 'Towers of Silence', 'Wolf Spiders', 'Mustard Gas', 'Anthropodermic Bibliopegy'].every(anchor => INSPIRATION_CARDS.find(card => card.anchor === anchor).imageUrl), 'Provided inspiration images must be mapped to their cards.');
    console.assert(typeof renderComponentMetaList === 'function', 'Component cards must render structured metadata instead of loose tag pills.');
    console.assert(renderComponentMetaList(COMPONENTS[0]).includes('Inspiration') && renderComponentMetaList(COMPONENTS[0]).includes('Themes') && renderComponentMetaList(COMPONENTS[0]).includes('Motifs'), 'Component metadata must be grouped by Inspiration, Theme(s), and Motif(s).');
    console.assert(renderComponentMetaList({ ...COMPONENTS.find(component => component.id === 'tower-refuses-burial') }).includes('Height') && renderComponentMetaList({ ...COMPONENTS.find(component => component.id === 'tower-refuses-burial') }).includes('Sun'), 'Motif metadata must render in title case.');
    console.assert(document.documentElement.innerHTML.includes('component-toggle-btn') && document.documentElement.innerHTML.includes('expandedComponentId'), 'Component cards must use +/- controls and click-to-expand state.');
    console.assert(typeof resolveComponentTargetSlot === 'function' && typeof findComponentAssignedSlot === 'function', 'Component +/- buttons must resolve their compatible target slot automatically instead of depending on the currently selected slot.');
    console.assert(typeof startComponentDrag === 'function' && typeof handleSlotDrop === 'function' && typeof placeComponentInSlot === 'function', 'Component cards must support drag-and-drop from the gallery into compatible Crucible slots.');
    console.assert(document.documentElement.innerHTML.includes('draggable="true"') && document.documentElement.innerHTML.includes('is-dragging-component') && document.documentElement.innerHTML.includes('drop-compatible') && document.documentElement.innerHTML.includes('drop-incompatible'), 'Dragging a component must highlight compatible slots and dim incompatible slots.');
    console.assert(document.documentElement.innerHTML.includes('.build-slot:hover') && document.documentElement.innerHTML.includes('transform: scale(1.003)') && document.documentElement.innerHTML.includes('.brief-choice-card:hover') && document.documentElement.innerHTML.includes('transform: scale(1.006)'), 'Hover motion should use subtle scale, not upward translation, for Crucible and Wizard cards.');
    console.assert(typeof openComponentContextMenu === 'function' && typeof showContextMenu === 'function', 'Component cards must expose a right-click context menu for add/remove and expand/collapse actions.');
    console.assert(document.documentElement.innerHTML.includes('height: calc(100dvh - 116px);') && document.documentElement.innerHTML.includes('align-items: stretch'), 'Crucible and Navigator columns must use equal viewport height on desktop.');
    console.assert(document.documentElement.innerHTML.includes('container-name: crucible') && document.documentElement.innerHTML.includes('@container crucible (min-width: 720px)') && typeof getSlotLayoutClass === 'function', 'Crucible slot layout must use container queries and per-slot layout classes.');
    console.assert(document.documentElement.innerHTML.includes('background-blend-mode: overlay, overlay, normal') && document.documentElement.innerHTML.includes('border: 2px solid color-mix'), 'Main glass panels must use overlay blend mode and 2px borders.');
    console.assert(document.documentElement.innerHTML.includes('linear-gradient(135deg, rgba(42, 39, 41, .14), transparent 70%)'), 'Empty build slots must use the darker, less milky background gradient.');
    console.assert(document.documentElement.innerHTML.includes('.workbench-resizer::after') && document.documentElement.innerHTML.includes('repeating-linear-gradient'), 'Workbench resizer must read visually as a draggable handle, not just a divider line.');
    console.assert(getSlotLayoutClass({ id: 'horrorPremise' }) === 'build-slot--wide' && getSlotLayoutClass({ id: 'sensoryLayer' }) === 'build-slot--wide' && getSlotLayoutClass({ id: 'hazard' }) === 'build-slot--half', 'Premise and Sensory Layer must stay full-width while ordinary slots can pair in columns.');
    console.assert(document.documentElement.innerHTML.includes('data-menu-action="component-toggle"') && document.documentElement.innerHTML.includes('data-menu-action="component-expand"'), 'Component context menu must provide component-toggle and component-expand actions.');
    console.assert(typeof renderBriefWizard === 'function' && typeof renderNavigatorFilterChip === 'function', 'Brief Wizard and Component Navigator filters must be implemented as separate systems.');
    console.assert(document.querySelector('.brief-wizard__foot #briefWizardProgress') && document.documentElement.innerHTML.includes('--brief-progress'), 'Brief Wizard stepper must live in the footer between navigation arrows and use a connected progress line.');
    console.assert(typeof getSelectedHorrors === 'function' && state.horrors instanceof Set, 'Turn Toward must support multiple horror selections while Start From remains single-choice.');
    console.assert(document.documentElement.innerHTML.includes("isHorrorSelected(value)") && document.documentElement.innerHTML.includes('toggleHorrorSelection(value)'), 'Navigator Turn Toward chips and wizard cards must use multi-select horror state.');
    console.assert(document.documentElement.innerHTML.includes('grid-template-columns: repeat(4, 92px)') && document.documentElement.innerHTML.includes('left: 46px;') && document.documentElement.innerHTML.includes('100% - 92px'), 'Brief Wizard stepper line must align to the center of wider labeled step boxes.');
    console.assert(document.documentElement.innerHTML.includes('brief-step-number') && document.documentElement.innerHTML.includes('brief-step-label'), 'Brief Wizard stepper must show a square number box with the step title beneath it.');
    console.assert(document.documentElement.innerHTML.includes('html::-webkit-scrollbar') && document.documentElement.innerHTML.includes('width: 15px;'), 'Document scrollbar must remain visually heavier than internal scrollbars.');
    console.assert(document.documentElement.innerHTML.includes('.build-canvas::-webkit-scrollbar') && document.documentElement.innerHTML.includes('width: 6px;') && document.documentElement.innerHTML.includes('background: transparent;'), 'Internal scrollbars must be thin, squared, transparent-track custom scrollbars.');
    console.assert(document.documentElement.innerHTML.includes('.brief-wizard__body::-webkit-scrollbar') && document.documentElement.innerHTML.includes('.brief-wizard__body::-webkit-scrollbar-thumb'), 'Brief Wizard body must use the same custom internal scrollbar styling as the rest of the site.');
    const inspirationBodyStart = document.documentElement.innerHTML.indexOf('.inspiration-body {');
    const inspirationBodyEnd = document.documentElement.innerHTML.indexOf('.inspiration-body h3', inspirationBodyStart);
    const inspirationBodyCss = inspirationBodyStart >= 0 && inspirationBodyEnd > inspirationBodyStart ? document.documentElement.innerHTML.slice(inspirationBodyStart, inspirationBodyEnd) : '';
    console.assert(inspirationBodyCss.includes('position: absolute') && inspirationBodyCss.includes('backdrop-filter: blur(2px) saturate(118%)') && inspirationBodyCss.includes('-webkit-backdrop-filter: blur(2px) saturate(118%)'), 'Inspiration card body must exist, stay positioned over the card, and keep subtle blur.');
    console.assert(!document.documentElement.innerHTML.includes('scrollbar-width: thin;'), 'Internal scrollbars must not use standard scrollbar-width because it can override WebKit scrollbar styling.');
    console.assert(!document.documentElement.innerHTML.includes('scrollbar-color: rgba(122, 26, 40, .82)'), 'Root scrollbar-color must not be set because it can inherit into internal scroll containers and suppress WebKit scrollbar rendering.');
    console.assert(document.documentElement.innerHTML.includes('.panel.build-canvas::-webkit-scrollbar') && document.documentElement.innerHTML.includes('scrollbar-color: auto;'), 'Panel scroll containers must explicitly reset standard scrollbar properties and use WebKit pseudo-elements.');
    console.assert(document.documentElement.innerHTML.includes('.component-card.in-build') && document.documentElement.innerHTML.includes('0 0 46px rgba(51, 3, 12, .6)'), 'In-build component cards must use the same active visual language as active Crucible slots.');
    console.assert(els.readModeSwitch && state.readAloudMode === 'compact' && typeof toggleReadAloudMode === 'function', 'Compiled output must support Compact / Extended read-aloud mode through a single switch control.');
    console.assert(document.querySelector('.build-canvas-shell > .panel.build-canvas') && document.querySelector('.build-canvas-shell > .canvas-icon-actions') && !document.querySelector('.canvas-actions .canvas-icon-actions'), 'Canvas icon actions must be outside the scrollable build-canvas content and outside .canvas-actions.');
    console.assert(document.getElementById('savedBuildsBtn') && document.getElementById('savedBuildsModal') && document.querySelector('.brief-actions #savedBuildsBtn') && !document.querySelector('.build-canvas .saved-block') && !document.querySelector('.build-canvas details .saved-list'), 'Saved builds must open from a Crucible action button and render in a modal, not inside the build canvas.');
    console.assert(document.getElementById('regionsPanel') && typeof renderLocationRegions === 'function' && typeof generateLocationRegions === 'function' && typeof buildCompiledMarkdownRegionSections === 'function', 'Darken a Location must support MVP Location Regions with generated rooms/regions and compiled output.');
    console.assert(typeof isRegionTemplateCompatibleWithContext === 'function' && typeof getContextCompatibleRegionTemplates === 'function', 'Location Region generation must use hard context compatibility, not only weighted scoring.');
    console.assert(isRegionTemplateCompatibleWithContext(LOCATION_REGION_TEMPLATES.find(region => region.templateId === 'wax-portrait-room'), 'Noble House') && !isRegionTemplateCompatibleWithContext(LOCATION_REGION_TEMPLATES.find(region => region.templateId === 'wax-portrait-room'), 'Forest') && !isRegionTemplateCompatibleWithContext(LOCATION_REGION_TEMPLATES.find(region => region.templateId === 'wax-portrait-room'), 'Cave'), 'Built-room regions such as Wax Portrait Room must not appear in natural-context random generation.');
    console.assert(Array.isArray(LOCATION_REGION_TEMPLATES) && LOCATION_REGION_TEMPLATES.length >= 8 && LOCATION_REGION_TEMPLATES.every(region => region.shape && region.connectors && region.readAloud && region.feature && region.interaction && region.danger && region.secret), 'Location Region templates must include map-oriented metadata and table-ready exploration content.');
    console.assert(document.documentElement.innerHTML.includes('--canvas-action-overlap') && document.documentElement.innerHTML.includes('bottom: calc(-1 * var(--canvas-action-overlap))') && document.documentElement.innerHTML.includes('flex-direction: row') && document.documentElement.innerHTML.includes('.canvas-icon-actions .icon-btn'), 'Canvas icon actions must sit horizontally against the lower build-canvas border with a half-in / half-out overlap.');
    console.assert(document.documentElement.innerHTML.includes('.read-mode-switch__knob') && document.documentElement.innerHTML.includes('.read-mode-toggle.is-extended .read-mode-switch') && !document.documentElement.innerHTML.includes('readModeCompactBtn'), 'Read-aloud control must render as a central switch with non-interactive Compact / Extended labels, not two segmented buttons.');
    console.assert(typeof normalizeTextBlocks === 'function' && typeof buildOpeningReadAloud === 'function' && typeof buildCompiledMarkdownTextSections === 'function', 'Read-aloud text must be organized by table-use role instead of blindly concatenating component tableText.');
    console.assert(document.documentElement.innerHTML.includes('Opening Read-Aloud') && document.documentElement.innerHTML.includes('Triggered Text') && document.documentElement.innerHTML.includes('GM Notes'), 'Compiled output must separate opening text, triggered text, investigation/reveal text, consequences, and GM notes.');
  }
  ...
  function attachEvents() {
    ...
    els.briefWizardModal.addEventListener('click', event => { if (event.target === els.briefWizardModal) closeBriefWizard(); });
    els.contextComboBtn.addEventListener('click', event => { event.stopPropagation(); toggleBriefPanel('context'); });
    els.horrorComboBtn.addEventListener('click', event => { event.stopPropagation(); toggleBriefPanel('horror'); });
    els.contextComboSearch.addEventListener('input', () => { state.contextSearch = els.contextComboSearch.value.trim().toLowerCase(); renderBriefControls(); });
    els.horrorComboSearch.addEventListener('input', () => { state.horrorSearch = els.horrorComboSearch.value.trim().toLowerCase(); renderBriefControls(); });
    ...
    els.sourcePickerSearchInput.addEventListener('input', () => { state.sourcePickerSearch = els.sourcePickerSearchInput.value.trim().toLowerCase(); renderSourcePicker(); });
    els.sourceTypeFilterBtn.addEventListener('click', event => { event.stopPropagation(); toggleSourceFilterPanel('type'); });
    els.sourceThemeFilterBtn.addEventListener('click', event => { event.stopPropagation(); toggleSourceFilterPanel('theme'); });
    els.sourceTypeFilterSearch.addEventListener('input', () => { state.sourceTypeSearch = els.sourceTypeFilterSearch.value.trim().toLowerCase(); renderSourceFilterControls(); });
    els.sourceThemeFilterSearch.addEventListener('input', () => { state.sourceThemeSearch = els.sourceThemeFilterSearch.value.trim().toLowerCase(); renderSourceFilterControls(); });
    ...
    if (els.inspirationDetailModal) {
      els.inspirationDetailModal.addEventListener('click', event => { if (event.target === els.inspirationDetailModal) closeInspirationDetail(); });
    }
    ...
    els.searchInput.addEventListener('input', () => { state.search = els.searchInput.value.trim().toLowerCase(); renderNavigator(); });
    ...
    els.savedBuildsModal.addEventListener('click', event => { if (event.target === els.savedBuildsModal) closeSavedBuildsModal(); });
  ...
  function resetBuildForWorkflow() {
    ...
    slots.forEach(slot => { state.build[slot.id] = []; });
    state.activeSlot = slots[0].id || null;
    state.locationRegions = [];
  }

  function setDefaultTitle() { els.buildTitleInput.value = WORKFLOWS[state.workflow].defaultTitle; }
  ...
  function hydrateIconButtonTooltips(root = document) {
    root.querySelectorAll('.icon-btn, .slot-menu-btn, .component-toggle-btn, .navigator-filter-btn, .mode-btn').forEach(button => {
  ...
  function renderBriefSummary() {
    ...
    const sourceLabel = selectedSources.length ? formatSelectedSourcesLabel(selectedSources) : 'Any Source';
  ...
  function renderBriefWizard(options = {}) {
    ...
    const steps = [
      { title: 'Start From', subtitle: 'Choose the scene, place, creature, reward, or clue you already have.' },
      { title: 'Turn Toward', subtitle: 'Choose the horror language the Crucible should privilege.' },
      { title: 'Draw From', subtitle: 'Choose one or more inspirations behind the component pool.' },
      { title: 'Hit With', subtitle: 'Choose how strongly the horror should affect play.' }
    ];
  ...
  function attachBriefWizardStepEvents() {
    els.briefWizardBody.querySelectorAll('[data-brief-context]').forEach(button => button.addEventListener('click', () => {
      state.context = button.dataset.briefContext;
      afterBriefWizardSelection();
    }));
    els.briefWizardBody.querySelectorAll('[data-brief-horror]').forEach(button => button.addEventListener('click', () => {
      toggleHorrorSelection(button.dataset.briefHorror);
      state.tags.clear();
      afterBriefWizardSelection({ stay: true });
    }));
    els.briefWizardBody.querySelectorAll('[data-brief-source]').forEach(button => button.addEventListener('click', () => {
    ...
    els.briefWizardBody.querySelectorAll('[data-brief-intrusion]').forEach(button => button.addEventListener('click', () => {
  ...
  function getSelectedHorrors() {
    if (state.horrors instanceof Set && state.horrors.size) return Array.from(state.horrors).filter(value => HORROR_TYPES.includes(value) && value !== 'Any');
  ...
  function toggleHorrorSelection(value) {
    if (!state.horrors || !(state.horrors instanceof Set)) state.horrors = new Set(getSelectedHorrors());
    if (value === 'Any') {
      state.horrors.clear();
      state.horror = 'Any';
      return;
  ...
  function setHorrorSelection(values) {
    const selected = (Array.isArray(values) ? values : [values]).filter(value => HORROR_TYPES.includes(value) && value !== 'Any');
    state.horrors = new Set(selected);
    state.horror = selected[0] || 'Any';
  }

  function getContextIcon(value) {
    const icons = { Any: 'fa-asterisk', Cave: 'fa-mountain', Crypt: 'fa-box-archive', Chapel: 'fa-church', Forest: 'fa-tree', Mine: 'fa-helmet-safety', 'Noble House': 'fa-landmark', Village: 'fa-house-chimney', Ruins: 'fa-archway' };
    return icons[value] || 'fa-cube';
  }

  function getHorrorIcon(value) {
    const icons = { Any: 'fa-asterisk', Gothic: 'fa-crow', 'Body Horror': 'fa-dna', 'Religious Horror': 'fa-church', 'Folk Horror': 'fa-wheat-awn', 'War Horror': 'fa-skull-crossbones', 'Cosmic Horror': 'fa-meteor', 'Disease Horror': 'fa-virus', 'Occult Horror': 'fa-eye', 'Psychological Horror': 'fa-brain' };
    return icons[value] || 'fa-mask';
  }

  function getIntrusionIcon(value) {
    const icons = { Any: 'fa-asterisk', Low: 'fa-feather', Medium: 'fa-bolt', High: 'fa-fire' };
  ...
  function renderBriefCombobox(config) {
    ...
    config.list.innerHTML = values.length ? values.map(value => `<button class="brief-combobox__option ${value === config.activeValue ? 'active' : ''}" type="button" role="option" aria-selected="${value === config.activeValue ? 'true' : 'false'}" data-brief-value="${escapeHtml(value)}">${escapeHtml(value)}</button>`).join('') : '<div class="empty">No matches.</div>';
    config.list.querySelectorAll('[data-brief-value]').forEach(button => button.addEventListener('click', event => {
      event.stopPropagation();
  ...
  function renderIntrusionSegments() {
    ...
    els.intrusionSegments.innerHTML = INTRUSION_LEVELS.map(value => `<button class="intrusion-segment ${state.intrusion === value ? 'active' : ''}" type="button" role="radio" aria-checked="${state.intrusion === value ? 'true' : 'false'}" data-intrusion-value="${escapeHtml(value)}" title="${escapeHtml(INTRUSION_LABELS[value] || value)}">${escapeHtml(value)}</button>`).join('');
  }
  ...
  function renderWorkflowButtons() {
    ...
    els.workflowButtons.innerHTML = Object.entries(WORKFLOWS).map(([value, workflow]) => `
  ...
  function renderBuildSlots() {
    ...
    els.buildSlots.innerHTML = slots.map(slot => {
    ...
    els.buildSlots.querySelectorAll('[data-select-slot]').forEach(node => {
      ...
      node.querySelectorAll('[data-select-sensory-kind]').forEach(subslot => subslot.addEventListener('keydown', event => {
    ...
    els.buildSlots.querySelectorAll('[data-slot-menu]').forEach(button => button.addEventListener('click', event => {
      event.preventDefault();
      event.stopPropagation();
    ...
    els.buildSlots.querySelectorAll('[data-select-slot]').forEach(node => attachSlotDropTarget(node, node.dataset.selectSlot));
    els.buildSlots.querySelectorAll('[data-select-sensory-kind]').forEach(subslot => attachSlotDropTarget(subslot, 'sensoryLayer', subslot.dataset.selectSensoryKind));
    els.buildSlots.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => handleSlotAction(button)));
  ...
  function renderSlotItem(slotId, item, max, options = {}) {
    const sensoryLabel = slotId === 'sensoryLayer' && item.sensoryKind && !options.hideSensoryKind ? `<span class="slot-item-kind">${escapeHtml(item.sensoryKind)}</span>` : '';
  ...
  function renderLocationRegions() {
    ...
    `;
    els.regionsPanel.querySelector('[data-region-action="add"]').addEventListener('click', () => addLocationRegion());
    els.regionsPanel.querySelector('[data-region-action="generate"]').addEventListener('click', () => generateLocationRegions(5));
    els.regionsPanel.querySelector('[data-region-action="clear"]').addEventListener('click', clearLocationRegions);
    els.regionsPanel.querySelectorAll('[data-region-remove]').forEach(button => button.addEventListener('click', () => removeLocationRegion(button.dataset.regionRemove)));
    els.regionsPanel.querySelectorAll('[data-region-regenerate]').forEach(button => button.addEventListener('click', () => regenerateLocationRegion(button.dataset.regionRegenerate)));
  ...
  function createDarkenLocationSnapshot() {
    return {
      ...
      locationRegions: getActiveLocationRegions().map(region => ({ ...region })),
  ...
  function sanitizeLocationRegions(regions) {
    return (Array.isArray(regions) ? regions : []).map(region => {
      const template = LOCATION_REGION_TEMPLATES.find(item => item.templateId === region.templateId) || LOCATION_REGION_TEMPLATES.find(item => item.name === region.name);
      return template ? { ...createLocationRegion(template), ...region, links: Array.isArray(region.links) ? region.links : template.links } : null;
    }).filter(Boolean);
  }
  ...
  function getContextCompatibleRegionTemplates(excludedTemplateIds = new Set()) {
    return LOCATION_REGION_TEMPLATES.filter(template => !excludedTemplateIds.has(template.templateId) && isRegionTemplateCompatibleWithContext(template));
  }
  ...
  function scoreRegionTemplate(template) {
    ...
    const sourceMatches = getSelectedSources().filter(anchor => template.sourceAnchors.includes(anchor));
  ...
  function pickLocationRegionTemplate(excludedTemplateIds = new Set()) {
    ...
    const pool = available.sort((a, b) => scoreRegionTemplate(b) - scoreRegionTemplate(a)).slice(0, 6);
  ...
  function generateLocationRegions(count = 5) {
    ...
    renderLocationRegions();
    refreshOutputIfOpen();
    if (regions.length < count && state.context !== 'Any') setStatus(`${regions.length} context-fit regions generated for ${state.context}. Add more templates for a full 5-room set.`);
  ...
  function regenerateLocationRegion(regionId) {
    ...
    if (!region) return;
    const used = new Set(current.filter(item => item.id !== regionId).map(item => item.templateId));
    const template = pickLocationRegionTemplate(used);
    if (!template) {
      setStatus(`No context-fit alternative available for ${state.context}.`);
      return;
    }
    state.locationRegions = current.map(item => item.id === regionId ? createLocationRegion(template) : item);
    renderLocationRegions();
    refreshOutputIfOpen();
  ...
  function getDraggedComponent(event) {
    const id = state.dragComponentId || event.dataTransfer.getData('application/x-cruor-component') || event.dataTransfer.getData('text/plain') || '';
  ...
  function handleSlotDrop(event, slotId, sensoryKind = '') {
    ...
    state.activeSensoryKind = slotId === 'sensoryLayer' ? (sensoryKind || component.sensoryKind || '') : '';
  ...
  function updateDropTargets(component) {
    ...
    els.buildSlots.querySelectorAll('[data-select-sensory-kind]').forEach(node => {
      const compatible = isComponentCompatibleWithDropTarget(component, 'sensoryLayer', node.dataset.selectSensoryKind);
      node.classList.toggle('drop-compatible', compatible);
      node.classList.toggle('drop-incompatible', !compatible);
    });
  }
  ...
  function clearComponentDragState() {
    ...
    els.componentGrid.querySelectorAll('.component-card.dragging').forEach(card => card.classList.remove('dragging'));
    els.buildSlots.querySelectorAll('.drop-compatible, .drop-incompatible, .drop-target').forEach(node => node.classList.remove('drop-compatible', 'drop-incompatible', 'drop-target'));
  }
  ...
  function isComponentCompatibleWithDropTarget(component, slotId, sensoryKind = '') {
    ...
    if (slotId === 'sensoryLayer' && sensoryKind) return Boolean(component.sensoryKind && component.sensoryKind === sensoryKind);
    return true;
  }
  ...
  function placeComponentInSlot(component, slotId, sensoryKind = '') {
    ...
    if (assignedSlotId && assignedSlotId !== slotId) {
      state.build[assignedSlotId] = (state.build[assignedSlotId] || []).filter(item => item.id !== component.id);
    }
    ...
    if (slotId === 'sensoryLayer') {
      ...
      state.build[slotId] = [...current.filter(item => item.sensoryKind !== kind), component].slice(0, slot.max);
      return true;
    }
    if (slot.max === 1) {
  ...
  function handleBuildCanvasEmptyClick(event) {
    ...
    if (event.target.closest('.build-slot, .sensory-subslot, .slot-item, button, input, textarea, select, details, summary, .canvas-actions, .build-head, .read-mode-toggle')) return;
    clearActiveSlot();
  }
  ...
  function updateActiveSlotUi() {
    ...
    els.buildSlots.querySelectorAll('.sensory-subslot').forEach(subslotNode => {
      const isKindActive = state.activeSlot === 'sensoryLayer' && subslotNode.dataset.selectSensoryKind === state.activeSensoryKind;
  ...
  function attachWorkbenchResize() {
    ...
    const applyNavigatorWidth = width => {
      ...
      try { localStorage.setItem(CRUOR_COMPOSER_NAVIGATOR_WIDTH_KEY, String(clamped)); } catch (error) {}
    };

    let savedWidth = 0;
    try { savedWidth = Number(localStorage.getItem(CRUOR_COMPOSER_NAVIGATOR_WIDTH_KEY)); } catch (error) {}
    ...
    resizer.addEventListener('keydown', event => {
      ...
      event.preventDefault();
      const current = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--navigator-width')) || 390;
  ...
  function filteredComponents(options = {}) {
    ...
    return COMPONENTS.filter(component => {
      ...
      if (!options.ignoreSensoryKind && slotId === 'sensoryLayer' && state.activeSensoryKind && component.sensoryKind !== state.activeSensoryKind) return false;
      if (state.context !== 'Any' && !component.contexts.includes('Any') && !component.contexts.includes(state.context)) return false;
      const selectedHorrors = getSelectedHorrors();
      if (selectedHorrors.length && !component.horror.some(horror => selectedHorrors.includes(horror))) return false;
      if (!ignoreSource && state.sourceAnchors.size && !component.sourceAnchors.some(anchor => state.sourceAnchors.has(anchor))) return false;
      if (state.intrusion !== 'Any' && component.intrusion !== state.intrusion) return false;
      if (!ignoreTag && state.tags.size && !component.horror.some(tag => state.tags.has(tag))) return false;
      if (state.search) {
        const haystack = [component.title, component.type, component.summary, getComponentTableText(component, 'compact'), getComponentTableText(component, 'extended'), component.mechanics, component.narrative, ...component.contexts, ...component.horror, ...component.sourceAnchors, ...component.sourceTypes, ...component.themes, ...component.motifs].join(' ').toLowerCase();
  ...
  function renderInspirations() {
    ...
    els.inspirationsGrid.innerHTML = INSPIRATION_CARDS.map(card => {
      ...
      const count = COMPONENTS.filter(component => component.sourceAnchors.includes(card.anchor)).length;
      const isActive = state.sourceAnchors.has(card.anchor);
      return `<article class="inspiration-card ${isActive ? 'active' : ''}" data-inspiration-anchor="${escapeHtml(card.anchor)}" role="button" tabindex="0" aria-pressed="${isActive ? 'true' : 'false'}"><div class="inspiration-visual" role="img" aria-label="${escapeHtml(card.imageNote)}">${renderInspirationImage(card)}<i class="fa-solid ${escapeHtml(card.icon)}" aria-hidden="true" ${card.imageUrl ? 'hidden' : ''}></i></div><div class="inspiration-body"><h3>${escapeHtml(card.anchor)}</h3><p>${escapeHtml(card.caption)}</p><div class="inspiration-meta">${details.motifs.slice(0, 5).map(motif => `<span>${escapeHtml(motif)}</span>`).join('')}</div><div class="inspiration-count">${count} linked components</div></div></article>`;
    }).join('');
  ...
  function renderSourceSummary() {
    ...
    if (els.sourcePickerBtn) {
      ...
      els.sourcePickerBtn.setAttribute('aria-expanded', els.sourcePickerModal && !els.sourcePickerModal.hidden ? 'true' : 'false');
      els.sourcePickerBtn.setAttribute('aria-label', selected.length ? `Draw from: ${selected.join(', ')}` : 'Draw from: Any Source');
    }
  }
  ...
  function toggleSourceAnchor(anchor) {
    ...
    refreshOutputIfOpen();
    const selected = getSelectedSources();
    setStatus(selected.length ? `Drawing from ${formatSourceList(selected)}.` : 'Inspirations cleared.');
  }
  ...
  function openInspirationDetail(anchor) {
    ...
    if (!els.inspirationDetailModal || !els.inspirationDetailTitle || !els.inspirationDetailType || !els.inspirationDetailBody) return;
    ...
    els.inspirationDetailBody.querySelectorAll('[data-use-inspiration]').forEach(button => button.addEventListener('click', () => {
    ...
    els.inspirationDetailBody.querySelectorAll('[data-detail-component]').forEach(button => button.addEventListener('click', () => {
  ...
  function showLinkedComponent(component, anchor) {
    if (!component) return;
    state.workflow = component.workflows.includes(state.workflow) ? state.workflow : component.workflows[0];
    populateContexts();
    state.context = 'Any';
    state.horror = 'Any';
    state.intrusion = 'Any';
    ...
    state.activeSensoryKind = state.activeSlot === 'sensoryLayer' && component.sensoryKind ? component.sensoryKind : '';
    closeInspirationDetail();
    renderAll();
    const targetCard = Array.from(els.componentGrid.querySelectorAll('[data-component-card]')).find(card => card.dataset.componentCard === component.id);
  ...
  function renderSourcePicker() {
    ...
    els.sourceSelectedChips.innerHTML = selected.length
       ? selected.map(anchor => `<button class="source-chip-btn" type="button" data-remove-source="${escapeHtml(anchor)}">${escapeHtml(anchor)} <i class="fa-solid fa-xmark" aria-hidden="true"></i></button>`).join('')
      : '<span class="source-empty-chip">Any Source</span>';
    els.sourceSelectedChips.querySelectorAll('[data-remove-source]').forEach(button => button.addEventListener('click', () => toggleSourceAnchor(button.dataset.removeSource)));

    const cards = INSPIRATION_CARDS.filter(card => {
      const details = SOURCE_DETAILS[card.anchor] || { sourceType: '', themes: [], motifs: [] };
      if (state.sourceTypeFilter !== 'Any Type' && details.sourceType !== state.sourceTypeFilter) return false;
      if (state.sourceThemeFilter !== 'Any Theme' && !(details.themes || []).includes(state.sourceThemeFilter)) return false;
    ...
    els.sourcePickerGrid.innerHTML = cards.length ? cards.map(card => {
    ...
    }).join('') : '<div class="empty">No inspirations match these filters.</div>';
    els.sourcePickerGrid.querySelectorAll('[data-source-card]').forEach(card => card.addEventListener('click', () => toggleSourceAnchor(card.dataset.sourceCard)));
  }
  ...
  function renderSourceFilterControl(config) {
    ...
    if (config.searchInput) config.searchInput.value = config.searchValue;
    const query = String(config.searchValue || '').toLowerCase();
    const values = config.allValues.filter(value => !query || value.toLowerCase().includes(query));
    config.list.innerHTML = values.length ? values.map(value => `<button class="filter-combobox__option ${value === config.activeValue ? 'active' : ''}" type="button" role="option" aria-selected="${value === config.activeValue ? 'true' : 'false'}" data-filter-value="${escapeHtml(value)}">${escapeHtml(value)}</button>`).join('') : '<div class="empty">No matches.</div>';
    config.list.querySelectorAll('[data-filter-value]').forEach(button => button.addEventListener('click', event => {
      event.stopPropagation();
  ...
  function renderNavigator() {
    ...
    els.navigatorTitle.textContent = slot ? `Choose ${slot.id === 'sensoryLayer' && state.activeSensoryKind ? state.activeSensoryKind : slot.label}` : 'Choose a component';
    ...
    if (!components.length) {
      ...
      const ignoreIntrusionButton = els.componentGrid.querySelector('[data-ignore-intrusion-filter]');
      ...
      if (ignoreIntrusionButton) ignoreIntrusionButton.addEventListener('click', ignoreIntrusionFilter);
      hydrateIconButtonTooltips(els.componentGrid);
      return;
    }
    els.componentGrid.innerHTML = components.map(component => renderComponentCard(component)).join('');
    els.componentGrid.querySelectorAll('[data-component-card]').forEach(card => {
      ...
      card.addEventListener('dragstart', event => startComponentDrag(event, card.dataset.componentCard));
      card.addEventListener('dragend', endComponentDrag);
    });
    els.componentGrid.querySelectorAll('[data-component-toggle]').forEach(button => button.addEventListener('click', event => {
      event.preventDefault();
      event.stopPropagation();
      toggleComponentForActiveSlot(COMPONENTS.find(item => item.id === button.dataset.componentToggle));
    }));
    hydrateIconButtonTooltips(els.componentGrid);
  }
  ...
  function renderTagFilters() {
    ...
    const tags = Array.from(new Set(baseComponents.flatMap(component => component.horror))).sort();
    Array.from(state.tags).forEach(tag => { if (!tags.includes(tag)) state.tags.delete(tag); });
    ...
    els.tagFilterBtn.setAttribute('data-tooltip', activeCount ? `Component filters: ${activeCount} active` : 'Component filters');
    ...
    els.tagFilterRow.querySelectorAll('[data-navigator-filter]').forEach(button => button.addEventListener('click', event => {
      event.stopPropagation();
  ...
  function handleNavigatorFilterClick(kind, value) {
    ...
    if (kind === 'horror') { toggleHorrorSelection(value); state.tags.clear(); }
    if (kind === 'source') {
  ...
  function renderComponentCard(component) {
    ...
    const sensoryBadge = component.sensoryKind ? `<span class="component-sensory-kind" title="${escapeHtml(component.sensoryKind)}" aria-label="${escapeHtml(component.sensoryKind)} detail"><i class="fa-solid ${escapeHtml(getSensoryKindIcon(component.sensoryKind))}" aria-hidden="true"></i><span>${escapeHtml(component.sensoryKind)}</span></span>` : '';
  ...
  function renderComponentMetaList(component) {
    const rows = [
      { label: component.sourceAnchors.length === 1 ? 'Inspiration' : 'Inspirations', values: component.sourceAnchors.length ? component.sourceAnchors : ['Unanchored'], className: 'source-chip' },
      { label: component.horror.length === 1 ? 'Theme' : 'Themes', values: component.horror.slice(0, 3), className: 'strong-chip' },
      { label: component.motifs.length === 1 ? 'Motif' : 'Motifs', values: component.motifs.slice(0, 4), className: '', format: 'title' },
  ...
  function formatManualTitle(value) {
    const text = String(value || '').trim().split(' ').filter(Boolean).join(' ');
    if (!text) return '';
    const minorWords = new Set(['a', 'an', 'and', 'as', 'at', 'but', 'by', 'for', 'from', 'in', 'into', 'nor', 'of', 'on', 'or', 'over', 'per', 'the', 'to', 'under', 'vs', 'with']);
  ...
  function getMatchReasons(component) {
    const reasons = [];
    if (state.context !== 'Any' && component.contexts.includes(state.context)) reasons.push(state.context);
    getSelectedHorrors().filter(horror => component.horror.includes(horror)).slice(0, 2).forEach(horror => reasons.push(horror));
    const sourceMatches = getSelectedSources().filter(anchor => component.sourceAnchors.includes(anchor));
    sourceMatches.slice(0, 2).forEach(anchor => reasons.push(anchor));
    if (state.intrusion !== 'Any' && component.intrusion === state.intrusion) reasons.push(`${state.intrusion} intensity`);
    Array.from(state.tags).filter(tag => component.horror.includes(tag)).slice(0, 2).forEach(tag => reasons.push(tag));
    if (state.activeSlot === 'sensoryLayer' && state.activeSensoryKind && component.sensoryKind === state.activeSensoryKind) reasons.push(component.sensoryKind);
  ...
  function toggleComponentForActiveSlot(component) {
    ...
    if (assignedSlotId) {
      ...
      if (slot.id === 'sensoryLayer' && component.sensoryKind) setStatus(`Selected ${component.sensoryKind}: “${component.title}”.`);
  ...
  function findComponentAssignedSlot(component) {
    if (!component) return '';
    const workflowSlots = WORKFLOWS[state.workflow].slots;
    const assignedSlot = workflowSlots.find(slot => component.slots.includes(slot.id) && (state.build[slot.id] || []).some(item => item.id === component.id));
  ...
  function getComponentActionLabel(component) {
    ...
    if (targetSlot.id === 'sensoryLayer' && component.sensoryKind && current.some(item => item.sensoryKind === component.sensoryKind)) return `Replace current ${component.sensoryKind.toLowerCase()} detail`;
  ...
  function openComponentContextMenu(event, componentId) {
    ...
    const isExpanded = state.expandedComponentId === component.id;
    const actionLabel = getComponentActionLabel(component);
    const actionIcon = isInBuild ? 'fa-minus' : (actionLabel.startsWith('Replace') ? 'fa-right-left' : 'fa-plus');
    const detailLabel = isExpanded ? 'Collapse Details' : 'Expand Details';
    const detailIcon = isExpanded ? 'fa-compress' : 'fa-expand';
    showContextMenu(event, `<button type="button" data-menu-action="component-toggle" data-id="${component.id}"><i class="fa-solid ${actionIcon}" aria-hidden="true"></i>${escapeHtml(actionLabel)}</button><button type="button" data-menu-action="component-expand" data-id="${component.id}"><i class="fa-solid ${detailIcon}" aria-hidden="true"></i>${detailLabel}</button>`);
  }
  ...
  function showContextMenu(event, html) {
    ...
    els.contextMenu.querySelectorAll('[data-menu-action]').forEach(button => button.addEventListener('click', () => { handleContextMenuAction(button); hideContextMenu(); }));
  }
  ...
  function handleContextMenuAction(button) {
    ...
    if (action === 'component-toggle') toggleComponentForActiveSlot(COMPONENTS.find(item => item.id === button.dataset.id));
    if (action === 'component-expand') toggleExpandedComponentCard(button.dataset.id);
  }

  function toggleSlotLock(slotId) {
    if (state.lockedSlots.has(slotId)) { state.lockedSlots.delete(slotId); setStatus('Slot unlocked.'); }
    else { state.lockedSlots.add(slotId); setStatus('Slot locked. Quick Build will not replace it.'); }
    renderBuildSlots();
    refreshOutputIfOpen();
  }
  ...
  function hideContextMenu() { if (els.contextMenu) els.contextMenu.hidden = true; }
  ...
  function getSlotIcon(slotId) {
    const icons = {
      horrorPremise: 'fa-map', sensoryLayer: 'fa-ear-listen', visibleAnomaly: 'fa-eye', hazard: 'fa-triangle-exclamation', clue: 'fa-magnifying-glass', encounterTwist: 'fa-skull', reward: 'fa-gem', creatureCorruption: 'fa-dna', openingSign: 'fa-door-open', combatTwist: 'fa-bolt', bossPhase: 'fa-mask', lairEffect: 'fa-dungeon', deathEffect: 'fa-skull-crossbones', rewardType: 'fa-gift', power: 'fa-hand-fist', cost: 'fa-droplet', temptation: 'fa-hand-holding-heart', visibleSign: 'fa-eye', escalation: 'fa-arrow-trend-up', removal: 'fa-ban', clueForm: 'fa-scroll', reveal: 'fa-eye-low-vision', disturbance: 'fa-face-dizzy', falseReading: 'fa-masks-theater', mechanicalCheck: 'fa-dice-d20', followUp: 'fa-shoe-prints'
    };
  ...
  function regenerateSlot(slotId) {
    if (state.lockedSlots.has(slotId)) { setStatus('Unlock this slot before regenerating it.'); return; }
    const slot = WORKFLOWS[state.workflow].slots.find(item => item.id === slotId);
    const previousActiveSlot = state.activeSlot;
    state.activeSlot = slotId;
    const candidates = filteredComponents({ ignoreSensoryKind: true }).filter(candidate => !(state.build[slotId] || []).some(item => item.id === candidate.id));
    state.activeSlot = previousActiveSlot;
    if (!slot || !candidates.length) { setStatus('No alternative component available for this slot.'); return; }
  ...
  function quickBuild(randomize) {
    ...
    slots.forEach(slot => {
      ...
      const chosen = [];
      const count = Math.min(slot.max, randomize && slot.max > 1 ? Math.ceil(Math.random() * slot.max) : slot.max);
  while (chosen.length < count && pool.length) {
        const index = randomize ? Math.floor(Math.random() * pool.length) : 0;
        const [item] = pool.splice(index, 1);
        if (slot.id === 'sensoryLayer' && item.sensoryKind && chosen.some(existing => existing.sensoryKind === item.sensoryKind)) continue;
        chosen.push(item);
      }
      state.build[slot.id] = chosen;
    });
    setStatus(randomize ? 'Unlocked slots randomized.' : 'The Crucible has been filled. Locked slots preserved.');
    renderBuildSlots();
    renderNavigator();
  ...
  function scoreComponent(component) {
    ...
    const matchedHorrors = getSelectedHorrors().filter(horror => component.horror.includes(horror));
    ...
    const matchedSources = getSelectedSources().filter(anchor => component.sourceAnchors.includes(anchor));
  ...
  function showView(view) { state.view = view; renderCurrentView(); }
  ...
  function renderCurrentView() {
    ...
    if (state.view === 'compiled') {
      const output = compileBuild({ silent: true });
      els.compiledView.innerHTML = output ? renderCompiledPreview() : '<div class="empty">Add at least one component before compiling.</div>'; 
    }
  ...
  function compileBuild(options = {}) {
    ...
    const contextText = state.context === 'Any' ? 'any compatible context' : `a ${state.context.toLowerCase()}`;
    const selectedHorrors = getSelectedHorrors();
    const horrorText = selectedHorrors.length ? formatHorrorList(selectedHorrors).toLowerCase() : 'dark fantasy horror';
    const intrusionText = state.intrusion === 'Any' ? 'low-to-medium intrusion' : `${state.intrusion.toLowerCase()} intrusion`;
    const selectedSources = getSelectedSources();
    const sourceText = formatSourceList(selectedSources);
    const sourcePhrase = selectedSources.length ? `, drawing from ${formatSourceList(selectedSources)}` : '';
    const readAloud = buildOpeningReadAloud(compiledEntries);
    const mechanics = allItems.filter(item => item.mechanics).map(item => `- ${item.title}: ${item.mechanics}`).join(NL);
  ...
  function getTextValue(value, mode = state.readAloudMode) {
    if (!value) return '';
    if (typeof value === 'object') return String(value[mode] || value.extended || value.compact || '').trim();
  ...
  function getDefaultTextRole(entry) {
    ...
    if (['horrorPremise', 'sensoryLayer', 'visibleAnomaly', 'openingSign', 'creatureCorruption'].includes(slotId)) return 'initial';
    if (['hazard', 'encounterTwist', 'combatTwist', 'bossPhase', 'lairEffect', 'deathEffect', 'followUp', 'escalation'].includes(slotId)) return 'triggered';
    if (['clue', 'clueForm', 'reveal', 'disturbance', 'falseReading', 'mechanicalCheck'].includes(slotId)) return 'investigation';
    if (['reward', 'rewardType', 'power', 'cost', 'temptation', 'visibleSign', 'removal'].includes(slotId)) return 'consequence';
    return 'gm';
  }
  ...
  function getDefaultTrigger(entry) {
    ...
    if (slotId === 'encounterTwist' || slotId === 'combatTwist') return 'When combat starts or the scene turns violent';
    ...
    if (slotId === 'clue' || slotId === 'clueForm' || slotId === 'reveal' || slotId === 'disturbance') return 'When the party investigates the evidence';
  ...
  function buildOpeningReadAloud(entries) {
    ...
    return initialBlocks.map(block => getTextBlockText(block)).filter(Boolean).join(' ');
  }
  ...
  function buildCompiledMarkdownTextSections(entries, NL, BLANK) {
    ...
    if (groups.triggered.length) sections.push(`## Triggered Text${NL}${renderBlocks(groups.triggered)}`);
    if (groups.investigation.length) sections.push(`## Investigation / Reveals${NL}${renderBlocks(groups.investigation)}`);
    if (groups.consequence.length) sections.push(`## Consequences / Rewards${NL}${renderBlocks(groups.consequence)}`);
    const gmNotes = entries.map(entry => entry.item.narrative ? `**${entry.item.title}.** ${entry.item.narrative}` : null).filter(Boolean);
  ...
  function renderCompiledTextSections(entries) {
    ...
    if (groups.triggered.length) sections.push(`<section class="compiled-section"><h2>Triggered Text</h2><div class="compiled-stack">${renderBlocks(groups.triggered)}</div></section>`);
    if (groups.investigation.length) sections.push(`<section class="compiled-section"><h2>Investigation / Reveals</h2><div class="compiled-stack">${renderBlocks(groups.investigation)}</div></section>`);
    if (groups.consequence.length) sections.push(`<section class="compiled-section"><h2>Consequences / Rewards</h2><div class="compiled-stack">${renderBlocks(groups.consequence)}</div></section>`);
    const gmNotes = entries.filter(entry => entry.item.narrative).map(entry => `<article class="compiled-text-block"><span class="compiled-trigger">${escapeHtml(displayTitle(entry.item.title))}</span><p>${escapeHtml(entry.item.narrative)}</p></article>`).join('');
    if (gmNotes) sections.push(`<section class="compiled-section"><h2>GM Notes</h2><div class="compiled-stack">${gmNotes}</div></section>`);
  ...
  function buildCompiledMarkdownRegionSections(NL, BLANK) {
    ...
    const regionText = regions.map((region, index) => {
      const readAloud = getTextValue(region.readAloud) || region.feature;
  ...
  function buildReadAloud(items) {
    return items.map(item => getComponentTableText(item)).filter(Boolean).slice(0, 5).join(' ');
  }
  ...
  function buildDropInUse(workflow, slots) {
    if (state.workflow === 'location') {
      ...
      const parts = [];
      if (has('horrorPremise')) parts.push('Use the Premise as the location’s core identity, not as a new plot.');
      if (has('sensoryLayer')) parts.push('Read the sensory details when the party enters, pauses, or investigates.');
      if (has('visibleAnomaly')) parts.push('Show the visible anomaly before players understand what it means.');
      if (has('hazard')) parts.push('Trigger the Hazard when players touch, disturb, spill blood, make noise, or cross the wrong threshold.');
      if (has('clue')) parts.push('Use the Clue to help players make a concrete decision.');
      if (has('encounterTwist')) parts.push('Apply the Encounter Twist only if a fight already happens here.');
      if (has('reward')) parts.push('Use the Outcome as what remains after the scene: reward, cost, mark, or consequence.');
      return parts.length ? parts.join(' ') : 'Attach the selected components to rooms, clues, enemies, or rewards already present in the session.';
    }
  ...
  function renderCompiledPreview() {
    ...
    const compiledEntries = getCompiledEntries(slots);
    const allItems = compiledEntries.map(entry => entry.item);
    if (!allItems.length && !getActiveLocationRegions().length) return '<div class="empty">Add at least one component or region before compiling.</div>'; 
    const readAloud = buildOpeningReadAloud(compiledEntries);
    ...
    const selectedHorrors = getSelectedHorrors();
    const horrorText = selectedHorrors.length ? formatHorrorList(selectedHorrors) : 'Dark fantasy horror';
    const selectedSources = getSelectedSources();
    ...
    const mechanicCards = allItems.filter(item => item.mechanics).map(item => `<div class="compiled-mechanic"><strong>${escapeHtml(displayTitle(item.title))}</strong><p>${escapeHtml(item.mechanics)}</p></div>`).join('') || '<div class="compiled-note"><p>No mechanical payload selected.</p></div>';
    const dropInUse = buildDropInUse(workflow, slots);
  ...
  async function copyCompiled() {
    const output = compileBuild({ silent: true });
    if (!output) { setStatus('Add at least one component before copying.'); return; }
    try { await navigator.clipboard.writeText(output); setStatus('Compiled build copied.'); }
    catch (error) { els.compiledOutput.value = output; els.compiledOutput.select(); document.execCommand('copy'); setStatus('Compiled build selected/copied.'); }
  }
  ...
  function saveBuild() {
    const output = compileBuild({ silent: true });
    if (!output) { setStatus('Add at least one component before saving.'); return; }
    ...
    const save = {
      ...
      build: Object.fromEntries(Object.entries(state.build).map(([slotId, items]) => [slotId, items.map(item => item.id)])),
  ...
  function renderSavedBuilds() {
    const saves = getSavedBuilds();
    if (!saves.length) { els.savedList.innerHTML = '<div class="empty">No saved builds yet.</div>'; return; }
    els.savedList.innerHTML = saves.map(save => {
      const workflowLabel = WORKFLOWS[save.workflow]?.label || save.workflow || 'Unknown workflow';
    ...
    els.savedList.querySelectorAll('[data-load-save]').forEach(button => button.addEventListener('click', () => loadSavedBuild(button.dataset.loadSave)));
  }

  function formatSaveSources(save) {
    const sources = Array.isArray(save.sourceAnchors) ? save.sourceAnchors : (save.sourceAnchor && save.sourceAnchor !== 'Any Source' ? [save.sourceAnchor] : []);
  ...
  function loadSavedBuild(saveId) {
    ...
    if (!save) return;
    if (!WORKFLOWS[save.workflow]) { setStatus('Saved build uses an unavailable workflow.'); return; }
    state.workflow = save.workflow;
    state.context = save.context || 'Any';
    setHorrorSelection(Array.isArray(save.horrors) && save.horrors.length ? save.horrors : (HORROR_TYPES.includes(save.horror) ? [save.horror] : []));
    state.tags = new Set(save.tags || []);
    state.sourceAnchors = new Set((Array.isArray(save.sourceAnchors) ? save.sourceAnchors : (save.sourceAnchor && save.sourceAnchor !== 'Any Source' ? [save.sourceAnchor] : [])).filter(anchor => SOURCE_ANCHORS.includes(anchor)));
    ...
    resetBuildForWorkflow();
    const workflowSlots = WORKFLOWS[state.workflow].slots;
    state.activeSlot = workflowSlots.some(slot => slot.id === save.activeSlot) ? save.activeSlot : workflowSlots[0].id;
    state.activeSensoryKind = SENSORY_KINDS.includes(save.activeSensoryKind) ? save.activeSensoryKind : '';
    ...
    Object.entries(migrateSavedBuildSlots(save.build || {})).forEach(([slotId, ids]) => {
      state.build[slotId] = ids.map(id => COMPONENTS.find(component => component.id === id)).filter(Boolean);
    });
  ...
  function migrateSavedBuildSlots(build) {
    const slotMap = { skin: 'horrorPremise', sensory: 'sensoryLayer', visual: 'visibleAnomaly' };
  ...
  function getActiveSlot() { return WORKFLOWS[state.workflow].slots.find(slot => slot.id === state.activeSlot) || null; }
  ...
  function getSensoryKindIcon(kind) {
    const icons = { Sound: 'fa-ear-listen', Smell: 'fa-wind', Touch: 'fa-hand' };
  ...
  function setStatus(message) {
    ...
    setStatus._timer = setTimeout(() => { els.status.textContent = ''; }, 2800);
  }
  ...
  function formatInline(text) { return text.replace(/\*\*(.*)\*\*/g, (match, content) => `<strong>${content}</strong>`); }
