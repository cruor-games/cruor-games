...
import { MANUAL_OVERRIDE_SCHEMA_VERSION, createEmptyManualOverrides, normalizeStairTransition } from "./map-generator.state.js";
import { getContextKey } from "./map-generator.profile.js";
import { parseRegionLink } from "./map-generator.graph.js";
import { formatMapLevel, getAvailableMapLevels } from "./map-generator.layout.js";
...
function getOrganicCorridorCellKeys(generatedMap) {
  ...
  (generatedMap.corridors || []).filter(isOrganicCorridor).forEach((corridor) => {
    (corridor.floorCells || []).forEach((cell) => organicCells.add(cellKey(cell.x, cell.y)));
  });
  return organicCells;
}
...
export function validateGeneratedMap(generatedMap, sourceConfig = generatedMap.config) {
  const tests = [];
  const errors = [];
  const warnings = [];
...
export function runGoldenSeedChecks(baseConfig, manualOverrides = createEmptyManualOverrides()) {
...
export function buildFullStructuralTestSuite(generatedMap, config, exportValidation) {
